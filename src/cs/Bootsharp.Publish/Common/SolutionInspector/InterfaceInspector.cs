...
internal sealed class InterfaceInspector(
    Preferences prefs,
...
)
{
    ...
    public InterfaceMeta Inspect(Type interfaceType, InterfaceKind kind)
    {
        ...
        return new InterfaceMeta
        {
            ...
            Methods = interfaceType
                ...
                .Select(m => CreateMethod(m, kind, impl.full))
                .ToArray(),
        };
    }
    ...
    private MethodKind ResolveMethodKind(
        InterfaceKind iKind,
        MethodInfo info,
        string implMethodName
    )
    {
        if (iKind == InterfaceKind.Export)
            return MethodKind.Invokable;
        // TODO: This assumes event methods are always renamed via prefs, which may not be the case.
        if (implMethodName != info.Name)
            return MethodKind.Event;
